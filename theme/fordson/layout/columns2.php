...
$user_id = $USER->id; // Assuming $USER is available and contains the current user's ID.
$preference_name = 'drawer-open-nav';
$preference_value = 'some_value'; // Adjust this value as needed.

// Set the user preference using the appropriate method.
set_user_preference($preference_name, $preference_value, $user_id);
...
$navigation = $PAGE->navigation->get('page');

// Check if the navigation object exists and has a collection label
if ($navigation) {
    $collectionLabel = $navigation->find('type', navigation_node::TYPE_COLLECTION);
    if ($collectionLabel) {
        $firstcollectionlabel = $collectionLabel->get_text();
        $templatecontext['firstcollectionlabel'] = $firstcollectionlabel;
    }
}
